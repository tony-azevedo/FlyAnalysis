function [Dist, rw, tw] = dtw_WarpingDistance(r,t)
...
% t is the sniff signal you are testing
% rw is the warped r vector
% tw is the warped t vector
% pflag  plot flag: 1 (yes), 0(no)
%
% Version comments: Parvez Ahammad - 29th Dec, 2009.


[row,M]=size(r); if (row > M) M=row; r=r'; end;
[row,N]=size(t); if (row > N) N=row; t=t'; end;
% initialize the distance matrix with euclidean distance between signal time-points
d=(repmat(r',1,N)-repmat(t,M,1)).^2; 

D=zeros(size(d));

%   The first gridpoint is given this value
D(1,1)=d(1,1);

% The grid elements in D are filled using dynamic programming (below)
...
Dist=D(M,N); % this is the shape warping distance

% Now that the grid matrix is filled, work back through it to 
% extract the optimal warping path. The optimal path is stored
...
if pflag
    ...
    % --- Warped sniff signals
    figure('Name','DTW - warped sniff signals', 'NumberTitle','off');
