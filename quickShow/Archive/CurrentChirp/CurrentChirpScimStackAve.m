function h = CurrentChirpScimStackAve(h,handles,savetag,varargin)
...
redtraces = nan(size(repmat(handles.trial.scimStackTrace(:,1),1,length(trials))));
greentraces = nan(size(repmat(handles.trial.scimStackTrace(:,1),1,length(trials))));

for t = 1:length(trials)
    trial = load(fullfile(handles.dir,sprintf(handles.trialStem,trials(t))));
    y(:,t) = trial.(y_name)(1:length(x));
    redtraces(1:length(trial.scimStackTrace(:,1)),t) = ...
        trial.scimStackTrace(:,1)/mean(trial.scimStackTrace(exp_t<=0,1)) *100 -100;
    greentraces(1:length(trial.scimStackTrace(:,2)),t) = ...
        trial.scimStackTrace(:,2)/mean(trial.scimStackTrace(exp_t<=0,2))*100 -100;
