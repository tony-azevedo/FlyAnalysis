function [trial,details] = findForceProbe(trial, varargin)
%% Set an ROI that avoids the leg, just gets the probe
% Need this for now, will surround with debugging in the future
...
p_crn_scalar = barbar*p_crn_0';

p_crn_ = p_crn_scalar*barbar+mean(l,1);
...
for ep_idx = 1:size(moveEvalPntsAlong,2)
    ...
    for rc = 1:size(xy,2)
        if xy(1,rc)>0 && xy(2,rc)>0
            ProfileMat(ep_idx,pos_idxs_i(rc)) = smooshedframe(xy(2,rc),xy(1,rc));
...
linearidx = sub2ind(size(smooshedframe),Cmap(~isn),Rmap(~isn));
...
%% Now decide whether the current alignment suggests there is a bar?
...
%% If so, then proceed to find the right alignment
