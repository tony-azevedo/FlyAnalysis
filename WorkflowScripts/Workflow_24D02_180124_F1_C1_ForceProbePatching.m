%% ForceProbe patcing workflow 180124_F1_C1
trial = load('B:\Raw_Data\180124\180124_F1_C1\CurrentStep2T_Raw_180124_F1_C1_8.mat');
...
trial = load('B:\Raw_Data\180124\180124_F1_C1\EpiFlash2T_Raw_180124_F1_C1_8.mat');
[protocol,~,~,~,~,~,trialStem,~] = extractRawIdentifiers(trial.name);
...
trial = load(sprintf(trialStem,7));
showProbeImage(trial)
...
trial = load('B:\Raw_Data\180124\180124_F1_C1\PiezoRamp2T_Raw_180124_F1_C1_1.mat');
[protocol,~,~,~,~,~,trialStem,~] = extractRawIdentifiers(trial.name);

clear trials
trials{1} = 1:15; % beautiful vidoes of the leg with no bar, but not analyable yet
trials{2} = 16:30; 
% trials{3} = 46:60; 
Nsets = length(trials);
...
trial = load('B:\Raw_Data\180124\180124_F1_C1\EpiFlash2TTrain_Raw_180124_F1_C1_1.mat');
[protocol,~,~,~,~,~,trialStem,~] = extractRawIdentifiers(trial.name);
...
trial = load(sprintf(trialStem,16));
showProbeImage(trial)
