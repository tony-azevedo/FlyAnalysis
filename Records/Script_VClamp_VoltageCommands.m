...
for bt_ind = 1:length(blocktrials)
    ...
    line(x,current,'color',clr,'displayname',tg,'parent',pnl(1,1).select());
    line(x,voltage,'color',clr,'displayname',tg,'parent',pnl(1,2).select());
...
for fn_ind = 2:length(fns)
    ...
    line(voltage(x>i&x<f),sensitive_current(x>i&x<f),'color',clr,'displayname',tg,'parent',pnl(2).select());
...
fn = fullfile(savedir,[ac.name{1},'_',rampstims{r_ind} '_Currents.pdf']);
figure(fig)
...
fn = fullfile(savedir,[ac.name{1}, '_VoltageSubtraction.pdf']);
figure(fig)
eval(['export_fig ' regexprep(fn,'\sAzevedo',''' Azevedo''') ' -pdf -transparent']);
...
fn = fullfile(savedir,[ac.name{1}, '_VoltageSubtraction_onset.pdf']);
figure(fig)
eval(['export_fig ' regexprep(fn,'\sAzevedo',''' Azevedo''') ' -pdf -transparent']);
...
fn = fullfile(savedir,[ac.name{1}, '_VoltageSubtraction_offset.pdf']);
figure(fig)
eval(['export_fig ' regexprep(fn,'\sAzevedo',''' Azevedo''') ' -pdf -transparent']);
...
% a_ind = 2
    ...
    for f_ind = 1:size(trial_seeds,2)
        ...
        for t_ind = 2:size(trial_seeds,3)
            clr = clrs(t_ind,:);
        ...
        for t_ind = 1:size(trial_seeds,3)
            ...
            elseif t_ind <= size(trial_seeds,3)
                ...
                line(x(x>i&x<f),current_diff(x>i&x<f),'color',clr,'displayname',tg,'parent',pnl(f_ind,1+t_ind).select());
                for x_ind = 1:length(peaktimes)
    ...
    fn = fullfile(savedir,[ac.name{1}, '_VoltageSine_Currents_' regexprep(num2str(trial.params.amp),'\.','_') 'V.pdf']);
    figure(fig)
    eval(['export_fig ' regexprep(fn,'\sAzevedo',''' Azevedo''') ' -pdf -transparent']);
...
for a_ind = 1:size(trial_seeds,1)
    ...
    fn = fullfile(savedir,[ac.name{1}, '_VoltageSine_Conductances_' regexprep(num2str(trial.params.amp),'\.','_') 'V.pdf']);
    figure(fig)
    eval(['export_fig ' regexprep(fn,'\sAzevedo',''' Azevedo''') ' -pdf -transparent']);
...
for a_ind = 1:size(trial_seeds,1)
    ...
    fn = fullfile(savedir,[ac.name{1}, '_VoltageSine_Z_' regexprep(num2str(trial.params.amp),'\.','_') 'V.pdf']);
    figure(fig)
    eval(['export_fig ' regexprep(fn,'\sAzevedo',''' Azevedo''') ' -pdf -transparent']);
