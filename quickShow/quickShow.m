...
% Last Modified by GUIDE v2.5 14-May-2021 11:25:45
...
if ~isInCellDir(handles)
    ...
    for i = 1:length(a)
        ...
        if contains(char(65:90),a(i).name(1)) && ...
                ...
                ~sum(strcmp(protocols,a(i).name(1:ind(1)-1)))
            % protocols{end+1} = a(i).name(1:ind(1)-1); %#ok<AGROW> 
            % For now, do not include continuous data files
        end
    end
...
% --- Executes on button press in openCDR.
% --- Now using this fucntion to open CDR (ContinuousDataReader) and
% possibly go to the time in question.
% See Github commit: f9abfc8cebdbe0e938a567b30d3d42d7a99b3521
function openCDR_Callback(cdrbutt, eventdata, h) %#ok<*INUSL>

h = guidata(cdrbutt);
file = uigetfile('*.bin');
disp(file)
fprintf('file = ''%s'';\nTo create data reader, run: \n\tcdr = ContinuousDataReader(file);\n',file);
fprintf('\tcdr.extractProtocolFiles(trial);\n');
...
T = load(handles.prtclTableFileName);
T = T.T;
T.excluded(T.trial == handles.trial.params.trial) = handles.trial.excluded;
save(handles.prtclTableFileName,'T')
fprintf('Saving protocol data table\n')
handles.prtclTable = T;
...
if contains(handles.notesfilename,'control')
    edit(regexprep(handles.notesfilename,'control_',''))
end
...
if length(a)==2 && contains(a(2).name,'control')
    ntstr = strcat(fileread(fullfile(handles.dir,a(1).name)),'\n\n');
    a = a(2);
else
    ntrstr = '';
end
...
set(handles.infoPanel,'userdata',sprintf('%s\n\n%s',ntstr,fileread(handles.notesfilename)))
...
function extractContinuousData(hObject,eventdata,h)
h = guidata(hObject);
file = uigetfile('*.bin');
disp(file)
fprintf('file = ''%s'';\nTo create data reader, run: \n\tcdr = ContinuousDataReader(file);\n',file);
...
% for now, assume there is a single continuous trial and all the trials are
% in it.
extractProtocolData(file,h.trial);
