function fig = PiezoStepFam(fig,handles,savetag)
...
for bt_ind = 1:length(blocktrials);
    plot(ax1,x,y(:,bt_ind),'color',[0 1 0] + [ 0 -1 1]*(bt_ind-1)/(length(blocktrials)-1),'tag',savetag);
    plot(ax2,x,sgs(:,bt_ind),'color',[0 1 0] + [ 0 -1 1]*(bt_ind-1)/(length(blocktrials)-1),'tag',savetag);
end
axis(ax1,'tight')
