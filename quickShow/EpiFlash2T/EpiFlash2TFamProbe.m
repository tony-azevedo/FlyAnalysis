function h = EpiFlash2TFamProbe(h,handles,savetag)
...
for bt_ind = 1:length(blocktrials)
    ...
    trials = findLikeTrials('name',handles.trial.name,'datastruct',handles.prtclData);
    y = nan(length(x),length(trials));
    prbe = nan(length(trial.forceProbeStuff.CoM(:)),length(trials));
...
%         plot(ax3,ft,trial.forceProbeStuff.CoM(:),'color',clrs(bt_ind,:),'tag',savetag);  hold(ax3,'on')
        prbe(:,t_ind) = trial.forceProbeStuff.CoM(:);
    end 
    plot(ax1,x,mean(y,2),'color',clrs(bt_ind,:),'tag',savetag);  hold(ax1,'on')
    plot(ax3,ft,mean(prbe,2),'color',clrs(bt_ind,:),'tag',savetag);  hold(ax3,'on')
end
