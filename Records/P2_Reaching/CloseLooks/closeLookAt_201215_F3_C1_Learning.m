...
T{1,:} = {'201215_F3_C1', 'Hot-Cell-Gal4',            'pilot', 'LEDArduinoFlashControl','empty'};
...
%%
fpHm(ft,trials,forceProbe,TP.ArduinoDuration,target)
...
function [ax1] = fpHm(ft,trials,forceProbe,ad,target)
...
blcks = [1 find(diff(target(1,:))~=0 | diff(target(2,:)))+1];
btrgts_x = target(1,blcks);
btrgts_x2 = target(2,blcks);
...
for b = 1:length(blcks)
    ...
    trgclr = cmap(indx,:);
    rectangle(ax1,'Position',[ax1.XLim(1) t1 diff(ax1.XLim) diff([t1, t2])],'EdgeColor',trgclr,'LineWidth',4)
    rectangle(ax1,'Position',[ax1.XLim(1) t1 diff(ax1.XLim) diff([t1, t2])],'EdgeColor',[.2 .2 .2],'LineWidth',.5)
end
