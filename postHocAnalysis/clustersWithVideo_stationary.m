function h = clustersWithVideo_stationary(h,handles,savetag,varargin)
...
output_video = [figpath '\' filename(1:end-4) '_FRoi_' num2str(factor) 'X.avi'];
...
v = h.(invec2)(t>=t_win(1)&t<=t_win(2));
...
%         im = imshow(mov,scale,'initialmagnification',50,'parent',dispax);
        ...
        for roi_ind = 1:length(h.ROI)
            line(h.ROI{roi_ind}(:,1),h.ROI{roi_ind}(:,2),'parent',dispax,'color',clrs(roi_ind,:));
        end
        ...
        for roi_ind = 1:length(h.ROI)
            ycol = ['ycol' num2str(roi_ind)];
            eval([ycol ' = y2(:,roi_ind);']);
            avis(roi_ind) = plot(traceax2,frame_times,y2(:,roi_ind),'color',clrs(roi_ind,:));
            hold(traceax2,'on');

            avis(roi_ind).YDataSource = ycol;
        end
        %voltage.XDataSource = 'x';
        ...
        set(im,'CData',mov);
        if frame_times(kk)>=2*1/fps*factor && notscaled
            notscaled = 0;
