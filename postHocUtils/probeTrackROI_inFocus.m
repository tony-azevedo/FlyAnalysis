...
if length(locs)>2
    locs = locs(locs<1000 & locs>100);
end
if length(locs)==1 || all(diff(locs)>200)
    [~,loc] = min(abs(evalpnts_x(locs)));
    bar_idx_i = locs(loc);
    zro = evalpnts_x(bar_idx_i);
    pk = pks(loc);
elseif length(locs)==2 && diff(locs)>60 && diff(locs)<200
    %     leftloc = locs(1);
    %     rightloc =locs(2);
    %     leftpk = pks(1);
    %     rightpk = pks(2);
    %     bar_idx_i_left = leftloc;
    %
    [~,loc] = min(abs(evalpnts_x(locs)));

    [pk,pkbot] = min(filtered_mu(locs(1)+1:locs(2)));
    pkbot = pkbot+locs(1);
    bar_idx_i = pkbot;
    zro = evalpnts_x(bar_idx_i);
else
    [~,loc] = min(abs(evalpnts_x(locs)));
    idx = locs(loc);
    closepks = abs(locs-idx)>60 & abs(locs-idx)<200;
    closepks(locs==idx)=1;
    highclosepks = pks>(max(pks)-dark)/2+dark;
    if all(diff(locs)>200)
        % There is really only one peak
    elseif any(closepks & highclosepks)
        locs_ = locs(closepks & highclosepks);
        otherloc = find(abs(locs_-idx)>60 & abs(locs_-idx)<200);
        locs_ = sort(locs_([otherloc(:)',find(locs_==idx)]));
        
        [pk,pkbot] = min(filtered_mu(locs_(1)+1:locs_(end)));
        pkbot = pkbot+locs_(1);
        bar_idx_i = pkbot;
    end
end
...
if abs(zro_fit)>100
    ...
    if coef_(3)<.5*coef(3)
        ...
        zro_fit = coef(2);
        zro_fit = zro;
        % zro_fit = find(filtered_mu(bar_idx_i:end)<dark+(pk-dark)*0.5,1,'first')+20+zro; %add 20 on
        y = filtered_mu(threesigma);
        x_right = evalpnts_x(evalpnts_x>zro_fit);
        y_right = filtered_mu(evalpnts_x>zro_fit);
    end
else
    y = gaussian_1at0(coef,x);
    x_right = evalpnts_x(evalpnts_x>zro_fit);
...
%     end
    ...
    closepks = abs(locs-loc)>60 & abs(locs-loc)<200;
    closepks(locs==loc)=1;
    highclosepks = pks>(max(pks)-dark)/2+dark;
    if all(diff(locs)>200)
        % There is really only one peak
    elseif any(closepks & highclosepks)
        locs = locs(closepks & highclosepks);
        otherloc = find(abs(locs-loc)>60 & abs(locs-loc)<200);
        locs = sort(locs([otherloc(:)',find(locs==loc)]));
        
        [pk,pkbot] = min(filtered_mu(locs(1)+1:locs(end)));
        pkbot = pkbot+locs(1);
        loc = pkbot;
    end
