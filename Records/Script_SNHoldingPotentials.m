...
data = data.data;
trials = findLikeTrials('name',trial.name,'datastruct',data,'exclude',{'trialBlock','tags','holdingPotential'},'withtags',tag_to_match);
...
for t_ind = 1:length(trials)
    ...
    current_Inj(t_ind) = mean(trial.voltage(x>.6));
end
...
for t_ind = 1:length(trial_ex)
    ...
    plot(pnl(1,t_ind).select(),x(x>2 & x<2.3),trial.current(x>2 & x<2.3),...
        'color',colr);
    set(pnl(1,t_ind).select(),'ylim',pAylims,'xtick',[],'ytick',[],'xcolor',[1 1 1],'ycolor',[1 1 1])
    set(pnl(1,t_ind).select(),'xtick',[],'ytick',[],'xcolor',[1 1 1],'ycolor',[1 1 1])
    pnl(1,t_ind).title([num2str(inj_ex(t_ind)) ' mV']);
    
    [xcor, lags] = xcorr(trial.current(x>0.1)-mean(trial.current(x>0.1)),'unbiased');
    lags = lags/trial.params.sampratein;
    plot(pnl(2,t_ind).select(),lags(lags>-.05&lags<0.05),xcor(lags>-.05&lags<0.05),...
        'color',colr);
    set(pnl(2,t_ind).select(),'ylim',pA2ylims,'xtick',[],'ytick',[],'xcolor',[1 1 1],'ycolor',[1 1 1])
    %set(pnl(2,t_ind).select(),'xtick',[],'ytick',[],'xcolor',[1 1 1],'ycolor',[1 1 1])
...
pnl(1,1).ylabel('pA')
pnl(1,1).xlabel('s')
text(2,pAylims(1),Condition,'parent',pnl(1,1).select(),'verticalalignment','bottom');
...
fn = fullfile(savedir,[id dateID '_' flynum '_' cellnum '_' regexprep(Condition,{'\s',','},{'_',''}) '_I.pdf']);
pnl.fontname = 'Arial';
figure(f)
eval(['export_fig ' regexprep(fn,'\sAzevedo',''' Azevedo''') ' -pdf -transparent']);

fn = fullfile(savedir,[id dateID '_' flynum '_' cellnum '_' regexprep(Condition,{'\s',','},{'_',''}) '_ICorr.pdf']);
figure(f1)
eval(['export_fig ' regexprep(fn,'\sAzevedo',''' Azevedo''') ' -pdf -transparent']);
