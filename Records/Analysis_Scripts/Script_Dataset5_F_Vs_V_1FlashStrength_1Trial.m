...
for tr_idx = find(nums==num)
    plot(ax,prb(ft>-.1&ft<.4,tr_idx),y_ft(ft>-.1&ft<.4,tr_idx),'color',clrs(tr_idx,:)); hold(ax,'on')
end
% plot(ax,mean(prb,2),mean(y_ft,2),'color',[0 0 0],'marker','.','markeredgecolor',[0 0 0],'Linewidth',2);
