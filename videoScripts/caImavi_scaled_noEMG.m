...
output_video = [figpath '\' filename(1:end-4) sprintf('_FClusters%s_Bar.avi',sprintf('_%d',cluster_idx))];
...
displayf = figure;
set(displayf,'position',[100 10 960 768+100],'color',[0 0 0]);
dispax = axes('parent',displayf,'units','pixels','position',[0 100 960 768]);
...
% hold(traceax,'on');
traceax2 = axes('parent',displayf,'units','pixels','position',[0 0 960 100]);
hold(traceax2,'on');
...
while hasFrame(vid)
    ...
    if frame0
        ...
        for cl_idx = cluster_idx % 1:size(trial.clustertraces,2) % cluster_idx
