...
trial = load('B:\Raw_Data\171109\171109_F1_C1\EpiFlash2T_Raw_171109_F1_C1_6.mat');
...
%% Set threshold and ROI to determine pixels to try to cluster
for set = 1:Nsets
    trialnumlist = trials{set};
    ...
    for tr_idx = trialnumlist(1:3)
        ...
        if (~isfield(trial,'excluded') || ~trial.excluded) 
            tic
            fprintf('%s\n',trial.name);
            trial = batchAskAboutThreshold(trial);
    ...
    for tr_idx = trialnumlist(4:end)
        ...
        fprintf('Saving ROI and threshold in trial %s\n',num2str(tr_idx))
        save(trial.name,'-struct','trial')
    end
end
...
%% Find where the bar comes to rest (where the fly lets go of it) and then map to that location
