function newfig = CurrentStep2TForceProbeFam_withSpikeRate(fig,handles,savetag)
...
fr = nan(length(x),length(blocktrials));
...
for bt_ind = 1:length(blocktrials)
    ...
    spikes_ = nan(length(x),length(trials));
    ...
    for t_ind = 1:length(trials)
        ...
        spikes_(:,t_ind) = 0;
        spikes_(trial.spikes,t_ind) = 1;
        
    end
    
    DT = 10/300;
    fr(:,bt_ind) = firingRate(x,spikes_,10/300);
...
%     plot(ax2,x,voltage(:,bt_ind),'color',[0 1 0] + [ 0 -1 1]*(bt_ind-1)/max([(length(blocktrials)-1) 1]),'tag',savetag);
    plot(ax2,x,fr(:,bt_ind),'color',[0 1 0] + [ 0 -1 1]*(bt_ind-1)/max([(length(blocktrials)-1) 1]),'tag',savetag);
