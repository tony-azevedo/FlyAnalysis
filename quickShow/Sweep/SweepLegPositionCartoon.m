function h = SweepLegPositionCartoon(h,handles,savetag)
...
plot(fe,'color',[1 1 1]*.8)
for l = 1:size(lpi,2)
    plot(legax,l,fe(l),...
        'marker','o','markersize',2,'markerfacecolor',clrs(l,:),'markeredgecolor',clrs(l,:))
end
 
legax = subplot(2,2,3,'parent',h); hold(legax,'on');
plot(ti,'color',[1 1 1]*.8)
for l = 1:size(lpi,2)
    plot(l,ti(l),...
        'marker','o','markersize',2,'markerfacecolor',clrs(l,:),'markeredgecolor',clrs(l,:))
end
legax = subplot(2,2,4,'parent',h); hold(legax,'on');
plot(ta,'color',[1 1 1]*.8)
for l = 1:size(lpi,2)
    plot(l,ta(l),...
        'marker','o','markersize',2,'markerfacecolor',clrs(l,:),'markeredgecolor',clrs(l,:))
end
