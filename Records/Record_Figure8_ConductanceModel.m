...
%% play with compare V response amplitude with and without VGs

V0 = 4.4849E-4; % Vrest - where the model comes to rest with I=0
na0 = 1.6934e-10; % Resting Na conductance when I=0 1.7374e-10
k0 = 5.1157e-10; % Resting K conductance when I=0 5.7798e-10

dur = 1;                        % Chirp duration
dt = 0.00005; sr = 1/dt;         % Time step
t = (0:dt:dur-dt)';             % time vector
ramp = .04;                     % the stimulus ramps on and off
amp = 30E-12;                   % the amplitude of current injection
f_i = 70;
...
standardstim = sin(2*pi*f_i*t); % create a chirp from 0.3 to 400 Hz
standardstim = chirp(t,.3,dur,400);
...
% --- Run the Model ---
out_IClamp = B1DynamicalSystem(I,t,'V0',V0,'na0',na0,'k0',k0);
out_IClamp_constVG = B1DynamicalSystem_VG_const(I,t,'V0',V0,'na0',na0,'k0',k0);
out_IClamp_VGBlock = B1DynamicalSystem_VGBlock(I,t,'V0',V0,'na0',1.7374e-10,'k0',5.7798e-10);

out = out_IClamp; fig = 6;
out = out_IClamp_constVG; fig = 7;

mean(out_IClamp.na(t>.04&t<.09))
mean(out_IClamp.k(t>.04&t<.09))

figure(8)
plot(t,out_IClamp_VGBlock.V,'color',[0 0 0]); hold on
plot(t,out_IClamp.V,'color',[.7 1 .7]); hold on
plot(t,out_IClamp_constVG.V,'color',[0 .7 0]); hold off
...
% Plot stuff
figure(fig);
set(fig,'position',[164        -148        1609         964])
