...
if ~isfield(trial,'legPositions')
    error('This trial leg positions. Use responseWithVideo_2T_Probe instead');
end
...
tibangle = trial.legPositions.Tibia_Angle;
tibangle = tibangle(ft>=t_win(1)&ft<=t_win(2));
y3 = nan(size(tibangle));
...
%         im = imshow(mov,scale,'initialmagnification',50,'parent',dispax);
        ...
        % ---- plot leg Position -----
        tib_angle = plot(traceax3,x_ft,y3,'color',[1 0 0]);
        tib_angle.YDataSource = 'y3';
        %voltage.XDataSource = 'x';

        ylims3 = [min(tibangle)-1 max(tibangle)+1];
