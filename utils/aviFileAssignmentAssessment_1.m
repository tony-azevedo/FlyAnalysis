...
% if the raw file has a file attached, use that one
...
for idx = 1:length(umavitimestamp)
    ...
    if any(mavitrialnum==tidx(2)) && (isfield(trial,'imageFile') && exist(regexprep(trial.imageFile,'Acquisition','Raw_Data'),'file')==2) && ~weird_umavis(idx)
