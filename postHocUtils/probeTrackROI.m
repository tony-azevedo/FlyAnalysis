...
% Find the best guess for the bar location: peak of the mean near evalpnts_x = 0;
...
plot([evalpnts_x(lefthashadjust) evalpnts_x(lefthashadjust)],[0 max(filtered_mu)],'color',[1.2 1 1]*.8,'parent',profileAxes);
plot([evalpnts_x(righthashadjust) evalpnts_x(righthashadjust)],[0 max(filtered_mu)],'color',[1 1 1.2]*.8,'parent',profileAxes);
...
trial.forceProbeStuff.Origin = moveEvalPntsAlong(:,1);
