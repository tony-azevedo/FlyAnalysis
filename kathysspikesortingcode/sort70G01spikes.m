function data = sort70G01spikes(event)
...
function startup
    ...
    prompt = {'Date','Fly Number','Cell Number','Trial number'};
    defaultAns = {date,'1','1','1'};
    ...
    gd.date = inputStr{1}; 
    gd.flynumber=inputStr{2}; 
    gd.cellnumber = inputStr{3}; 
    gd.n = str2num(inputStr{4}); 
    
    % check input and load file
%     gd.analysis_path = ['C:\Allison\Electrophysiology\EPhys_Analysis\',gd.date,'\',...
%         gd.date,'_F',gd.flynumber,'_C',gd.cellnumber];
%     gd.data_path = ['C:\Allison\Electrophysiology\EPhys_Data\',gd.date,'\',...
%         gd.date,'_F',gd.flynumber,'_C',gd.cellnumber];
    gd.analysis_path = ['/Users/Allison/Documents/Lab/Electrophysiology/EPhys_Analysis/Reasonable Cells/',gd.date,'/',...
        gd.date,'_F',gd.flynumber,'_C',gd.cellnumber];
    gd.data_path = ['/Users/Allison/Documents/Lab/Electrophysiology/EPhys_Data/',gd.date,'/',...
        gd.date,'_F',gd.flynumber,'_C',gd.cellnumber];
    fid = fopen([gd.analysis_path,'/SortedCell_',gd.date,'_F',gd.flynumber,'_C',gd.cellnumber,'.mat']);
    if fid == -1
        fid2 = fopen([gd.data_path,'/WCwaveform_',gd.date,'_F',gd.flynumber,'_C',gd.cellnumber,'.mat']);
        if fid2 == -1
            fprintf('Could not find data\n');
            return;
        else
            fclose(fid2);
            gd.info = load([gd.data_path,'/WCwaveform_',gd.date,'_F',...
                gd.flynumber,'_C',gd.cellnumber,'.mat'],'data');
        end
    else
            fprintf('Loading previous sort...\n');   
            gd.info = load([gd.analysis_path,'/SortedCell_',gd.date,'_F',...
                gd.flynumber,'_C',gd.cellnumber,'.mat'],'data');
    end
...
function loadTrial(n)
    ...
    global gd
    disp(n)
    load([gd.data_path,'/Raw_WCwaveform_',gd.date,'_F',gd.flynumber,'_C',gd.cellnumber,'_',num2str(n)]);
    gd.v = voltage;
    gd.s = stim; gd.s = resample(gd.s,gd.info.data(gd.n).sampratein,gd.info.data(gd.n).samprateout);
...
function sortTrial
    ...
    gd.sparray(find(gd.s~=0)) = sparraystim(find(gd.s~=0));
    %gd.sparray(100+find(gd.s~=0)) = sparraystim(100+find(gd.s~=0));
...
function updateFigs(a)
    ...
    gd.zoomfig.raw = subplot(2,1,1); hold off; plot(t,gd.v,'b','lineWidth',1); ylabel('V(mV)');
        ...
        box off; set(gca,'TickDir','out'); 
        set(gca,'Xlim',a);
        title(sprintf('%s Fly %s Cell %s, trial %d of %d',gd.date,gd.flynumber,...
            gd.cellnumber,gd.n,length(gd.info.data)));
...
function saveData  
    ...
    if ~isdir(gd.analysis_path)
        mkdir(gd.analysis_path)
    end
    data = gd.info.data;
    save([gd.analysis_path,'/SortedCell_',gd.date,'_F',gd.flynumber,'_C',gd.cellnumber,'.mat'],'data');
...
function plotFigs
    ...
    set(gcf,'PaperPositionMode','auto');
    set(gd.trialfig.fig,'KeyPressFcn','sort70G01spikes(''keyboard'')');

   nextbutton = uicontrol(...
        'Parent',gd.trialfig.fig,...
        'Units','characters',...
        'Callback','sort70G01spikes(''next'')',...
    ...
    gd.trialfig.spthresh = uicontrol(...
        'Parent',gd.trialfig.fig,...
        'Units','characters',...
        'Callback','sort70G01spikes(''chspontth'')',...
    ...
    gd.trialfig.stthresh = uicontrol(...
        'Parent',gd.trialfig.fig,...
        'Units','characters',...
        'Callback','sort70G01spikes(''chstimth'')',...
    ...
    set(gcf,'PaperPositionMode','auto');
        t=[1:gd.info.data(gd.n).nsampin]'/gd.info.data(gd.n).sampratein; 
    set(gd.zoomfig.fig,'KeyPressFcn','sort70G01spikes(''keyboard'')');
    ...
    nextbitbutton = uicontrol(...
        ...
        'Units','characters',...
        'Callback','sort70G01spikes(''nextbit'')',...
    ...
    prevbitbutton = uicontrol(...
        'Parent',gd.zoomfig.fig,...
        'Units','characters',...
        'Callback','sort70G01spikes(''prevbit'')',...
        'Position',[8 3 10 2.6],...
        'String',{'Previous' },...
        'Tag','PrevButton');
    ...
    deletebutton = uicontrol(...
        'Parent',gd.zoomfig.fig,...
        'Units','characters',...
        'Callback','sort70G01spikes(''delete'')',...
