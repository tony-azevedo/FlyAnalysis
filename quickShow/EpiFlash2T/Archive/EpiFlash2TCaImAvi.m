function h = EpiFlash2TCaImAvi(h,handles,savetag)

if ~isfield(handles.trial,'clustertraces') || ~isfield(handles.trial,'forceProbeStuff')
    return
end
...
panl = panel(h);
panl.pack('v',{1/4 3/8 3/8})  % response panel, stimulus panel
...
CoM = CoM*x_hat;
origin = handles.trial.forceProbeStuff.Origin'*x_hat;

h2 = postHocExposure(trial,length(CoM));
frame_times = x(h2.exposure);

ax3 = panl(3).select();
plot(ax3,frame_times,CoM(1:length(frame_times)))
axis(ax3,'tight')
set([ax1,ax2,ax3],'xlim',[x(1) x(end)])
xlabel(ax3,'s')
