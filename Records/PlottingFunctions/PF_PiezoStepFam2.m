function fig = PF_PiezoStepFam2(pnl,handles,savetag)
...
clrs = parula(length(blocktrials));
for bt_ind = 1:length(blocktrials);
    plot(ax1,x,y(:,bt_ind),'color',clrs(bt_ind,:),'tag',savetag);
    plot(ax2,x,sgs(:,bt_ind),'color',clrs(bt_ind,:),'tag',savetag);
end
