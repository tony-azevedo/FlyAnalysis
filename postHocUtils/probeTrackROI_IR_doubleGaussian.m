...
% 4) fit a sum of 2 gaussians to this part of the curve. 
% Use the left hand side, and a bit of the right hand side.
coef = [zro,pk-dark,bar_sigma,pk-dark,bar_sigma];
x = evalpnts_x(threesigma);

coef = nlinfit(evalpnts_x(threesigma)',filtered_mu(threesigma)-dark,@doubleGaussian_bar_1at0,coef);
y = doubleGaussian_bar_1at0(coef,x);
coef_right = [coef(4), coef(1)+54, coef(5)];
x_right = evalpnts_x(evalpnts_x>coef(1)+54);
y_right = gaussian_1at0(coef_right,x_right);

plot(ax1,find(threesigma),y,'color',[0 .8 0])
plot(ax1,find(evalpnts_x>coef(1)+54),y_right,'color',[0.8500    0.3250    0.0980])
...
lefthash = find(y_right < 1E-2*coef_right(1) ,1,'first')+sum(evalpnts_x<=coef(1)+54);
% compare that hash to the location of the minimum between the peak and the
% right hand side
if filtered_mu(lefthash)>coef_right(1)+dark  % right side of the bar comes up too much
...
plot(evalpnts_x(evalpnts_x>zro-3*coef_right(3)),gaussian_1at0(coef_right,evalpnts_x(evalpnts_x>zro-3*coef_right(3))),'color',[0 0 1],'parent',profileAxes);
...
%     fprintf('Load: '),toc
    ...
    [pk,loc] = max(filtered_frame_mu(1:righthash)-dark);
