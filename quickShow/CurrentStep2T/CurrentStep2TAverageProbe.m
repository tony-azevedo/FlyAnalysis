function h = CurrentStep2TAverageProbe(h,handles,savetag)
...
ax = panl(1).select();
for t = 1:length(trials)
    plot(ax,x,y(:,t),'color',[1 .7 .7],'tag',savetag); hold on
...
ax = panl(2).select();
for t = 1:length(trials)
    plot(ax,x,y_2(:,t),'color',[1 .7 .7],'tag',savetag); hold on
...
ax = panl(3).select();
for t = 1:length(trials)
    plot(ax,ft,prb(:,t),'color',[1 .7 .7],'tag',savetag); hold on;
    plot(ax,ft,mean(prb,2),'color',[.7 0 0],'tag',savetag);
end
box(ax,'off');
set(ax,'TickDir','out');
xlim([-trial.params.preDurInSec  trial.params.stimDurInSec+trial.params.postDurInSec])
