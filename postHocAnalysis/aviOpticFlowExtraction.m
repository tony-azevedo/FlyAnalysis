function h = aviOpticFlowExtraction(h,handles,savetag,varargin)
...
extrapads = 3*fs;
voltage = [zeros(extrapads,1); h.voltage; zeros(extrapads,1)]; %%pad because there is no data at edges of "sweep"
current = [zeros(extrapads,1); h.current; zeros(extrapads,1)];
exposure = [zeros(extrapads,1); h.exposure; zeros(extrapads,1)];
