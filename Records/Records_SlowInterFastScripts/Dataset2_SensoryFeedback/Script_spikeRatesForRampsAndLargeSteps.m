...
for c = 1:length(cids)
    cid = cids{c};
    fprintf(1,'Cell %s\n',cid);
    cidx = strcmp(T_RmpStpSlowFR.CellID,cid);
    positions = unique(T_RmpStpSlowFR.Position(cidx));
    ...
    for position = positions(:)'
        ...
        displacements = unique(T_RmpStpSlowFR.Displacement(cidx&posidx));
        speeds = unique(T_RmpStpSlowFR.Speed(cidx&posidx));
        ...
        for displacement = displacements(:)'
            ...
            for speed = speeds(:)'
                ...
                ridx = cidx & posidx & spdidx & dspidx;
                if ~sum(ridx)
                    continue
                end
