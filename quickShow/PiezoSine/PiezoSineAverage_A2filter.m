function h = PiezoSineAverage_A2filter(h,handles,savetag,varargin)
...
lpFilt = designfilt('lowpassiir','FilterOrder',8,'PassbandFrequency',300,'PassbandRipple',0.2,'SampleRate',50e3);
setpref('FigureMaking','A2VoltageFilter',lpFilt);
lpFilt = designfilt('lowpassiir','FilterOrder',8,'PassbandFrequency',100,'PassbandRipple',0.2,'SampleRate',50e3);
setpref('FigureMaking','A2SEMVoltageFilter',lpFilt);
d1 = getpref('FigureMaking','A2VoltageFilter');
d2 = getpref('FigureMaking','A2SEMVoltageFilter');
if d1.SampleRate ~= trial.params.sampratein
    d1 = designfilt('lowpassiir','FilterOrder',8,'PassbandFrequency',300,'PassbandRipple',0.2,'SampleRate',trial.params.sampratein);
    d2 = designfilt('lowpassiir','FilterOrder',8,'PassbandFrequency',100,'PassbandRipple',0.2,'SampleRate',trial.params.sampratein);
end
...
y_ = mean(y,2);
base = mean(y_(x<0&x>-trial.params.preDurInSec+0.1));
y_ = filtfilt(d1,y_-mean(y_))+base;

plot(ax,x(x_win),y_(x_win),'color',[.7 0 0],'tag',savetag);

sem_up = filtfilt(d2,std(y,[],2)/sqrt(size(y,2)));
sem_down = y_-sem_up;
sem_up = y_+sem_up;

line(x(x_win),sem_up(x_win),'parent',ax,'color',[.7 .7 .7],'tag','sem_up');
line(x(x_win),sem_down(x_win),'parent',ax,'color',[.7 .7 .7],'tag','sem_up');
