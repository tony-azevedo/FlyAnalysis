function h = CurrentChirpScimLineAve(h,handles,savetag,varargin)
...
redtraces = repmat(handles.trial.scimLineTrace(:,1),1,length(trials));
greentraces = repmat(handles.trial.scimLineTrace(:,2),1,length(trials));

for t = 1:length(trials)
    trial = load(fullfile(handles.dir,sprintf(handles.trialStem,trials(t))));
    y(:,t) = trial.(y_name)(1:length(x));
    f = smooth(trial.scimLineTrace(:,1)/mean(trial.scimLineTrace(exp_t<=0,1)) *100 -100);
    %f = trial.scimLineTrace(:,1);
    redtraces(:,t) = f(1:length(exp_t));
    f = smooth(trial.scimLineTrace(:,2)/mean(trial.scimLineTrace(exp_t<=0,2))*100 -100);
    %f = trial.scimLineTrace(:,2);
    greentraces(:,t) = f(1:length(exp_t));
