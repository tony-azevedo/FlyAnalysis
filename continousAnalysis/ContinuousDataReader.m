...
classdef ContinuousDataReader < handle
    ...
    methods (Access = protected)
        ...
        function readHeader(obj,varargin)
            ...
            % check if there is such a directory. If not, ask about it and
            % alert
            if isempty(dir(fullfile(obj.D,obj.name)))
                tempd = pwd;
                yn = questdlg(sprintf('Directory %s not found. Use pwd - %s?',obj.D,tempd));
                if strcmp('Yes',yn)
                    obj.D = tempd;
                else
                    error('Navigate to correct directory and rerun CDR.')
                end
