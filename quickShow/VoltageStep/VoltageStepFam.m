function fig = VoltageStepFam(fig,handles,savetag)
...
for bt_ind = 1:length(blocktrials);
    plot(ax1,x,current(:,bt_ind),'color',[0 1 0] + [ 0 -1 1]*(bt_ind-1)/(length(blocktrials)-1),'tag',savetag);
    plot(ax2,x,voltage(:,bt_ind),'color',[0 1 0] + [ 0 -1 1]*(bt_ind-1)/(length(blocktrials)-1),'tag',savetag);
end
axis(ax1,'tight')
...
title(ax1,[dateID '_' flynum '_' cellnum '_' protocol '_Block' num2str(trial.params.trialBlock) '_' mfilename sprintf('_%s',tags{:})]);
