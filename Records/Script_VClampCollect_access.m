...
for af_idx = 1:length(access_figs)
    ...
    if median(access) < 0.06, 
        ...
        line(access_time-access_time(1),access,...
            'color',clrs(af_idx,:),...
            'linestyle','.',... 'marker','.','markerfacecolor',clrs(af_idx,:),'markeredgecolor',clrs(af_idx,:),...
            'displayname',cell_id,...
            'tag',cell_id,...
            'parent',pnl(2).select());
    end
...
figure(fig)
eval(['export_fig ' 'Access_Collection.pdf' ' -pdf -transparent']);

set(fig,'paperpositionMode','auto');
saveas(fig,'Access_Collection','fig');
