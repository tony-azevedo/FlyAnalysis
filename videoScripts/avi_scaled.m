%% Makes a video with image scaling
...
output_video = [figpath '\' filename(1:end-4) '_scaled.avi'];

writerObj = VideoWriter(output_video,'Motion JPEG AVI');
writerObj.Quality = 100;
% writerObj = VideoWriter(output_video,'Motion JPEG 2000');
% writerObj.LosslessCompression = true;
writerObj.FrameRate = fps;
open(writerObj);

displayf = figure;
set(displayf,'position',[100 10 960 768],'color',[0 0 0]);
dispax = axes('parent',displayf,'units','pixels','position',[0 0 960 768]);
