...
% T{1,:} = {'201211_F1_C1', 'WT_berlin',            'pilot', 'LEDArduinoFlashControl','empty'};
...
% T{1,:} = {'210105_F3_C1', 'Hot-Cell-Gal4',            'pilot', 'LEDArduinoFlashControl','empty'};
% T{1,:} = {'210105_F4_C1', 'Hot-Cell-Gal4',            'pilot', 'LEDArduinoFlashControl','empty'};
% T{1,:} = {'210108_F2_C1', 'Hot-Cell-Gal4',            'pilot', 'LEDArduinoFlashControl','empty'};
% T{1,:} = {'210111_F1_C1', 'Hot-Cell-Gal4',            'pilot', 'LEDArduinoFlashControl','empty'};
% T{1,:} = {'210113_F1_C1', 'Hot-Cell-Gal4',            'pilot', 'LEDArduinoFlashControl','empty'};
% T{1,:} = {'210113_F2_C1', 'Hot-Cell-Gal4',            'pilot', 'LEDArduinoFlashControl','empty'};
% T{1,:} = {'210113_F3_C1', 'Hot-Cell-Gal4',            'pilot', 'LEDArduinoFlashControl','empty'};
T{1,:} = {'210122_F1_C1', 'Hot-Cell-Gal4, no ATR',            'pilot', 'LEDArduinoFlashControl','empty'};
% T{1,:} = {'210125_F1_C1', 'Hot-Cell-Gal4, no ATR',            'pilot', 'LEDArduinoFlashControl','empty'};
T{1,:} = {'210125_F2_C1', 'Hot-Cell-Gal4, no ATR',            'pilot', 'LEDArduinoFlashControl','empty'};
% T{1,:} = {'210127_F1_C1', 'Hot-Cell-Gal4, no ATR',            'pilot', 'LEDArduinoFlashControl','empty'};
T{1,:} = {'210129_F1_C1', 'Hot-Cell-Gal4, no ATR',            'pilot', 'LEDArduinoFlashControl','empty'};
T{1,:} = {'210211_F1_C1', 'Hot-Cell-Gal4, no ATR',            'pilot', 'LEDFlashWithPiezoCueControl','empty'};
T{1,:} = {'210211_F1_C1', 'Hot-Cell-Gal4, no ATR',            'pilot', 'LEDFlashWithPiezoCueControl','empty'};
...
dur = min(TP.durSweep)*TP.sampratein(1);
...
for t = 1:length(trials)
    ...
    try target(1,t) = trial.target_location(1);
    catch e
        target(1,t) = 460;
        trial.target_location(1) = 460;
        trial.target_location(2) = 100;
...
resttrialstart = find(diff(TP.ndf==0)>0)+1;
resttrialend = find(diff(TP.ndf==0)<0);
% check if there are some rest trials to begin
if TP.ndf(1)==0
    resttrialstart = [1;resttrialstart];
    if length(resttrialend) == length(resttrialstart)
        blocks_idx = [[1; resttrialend + 1],[resttrialstart-1;length(TP.ndf)]];
    else
...
for r = 1:size(blocks_idx,1)
    for c = 1:size(blocks_idx,2)
        blocks(r,c) = TP.trial(blocks_idx(r,c));
    end
end
...
blcktable.IsFlexion = flexiontarget(:);

neutral = btrgts_x;
for b = 1:height(blcktable)
    if blcktable.IsFlexion(b)
        neutral(b) = blcktable.Target_max(b)+20;
    elseif ~blcktable.IsFlexion(b)
...
fpHm(ft,trials,forceProbe,TP.ArduinoDuration,blocks,flexiontarget,blcktable{:,[3,4]})

%% forceprobe Heat Map
%fpHm(ft,trials,forceProbe,TP.ArduinoDuration,blocks,true(size(flexiontarget)),blcktable{:,[3,4]},[40,500])
fpHm(ft,trials,forceProbe,TP.ArduinoDuration,blocks,flexiontarget,blcktable{:,[3,4]},[320,560])

%% Tagging trials with the outcome

% 1: Leg in target - dim short flash - fly moves
% 2: Leg in target - dim short flash - fly does not move
