...
for t = 1:length(trials)
    trial = load(fullfile(dir,sprintf(trialStem,trials(t))));
    sweep = trial.voltage;
...
xlim(ax1,[17,300]);
panl(2,1).ylabel('V^2/pA^2')
panl(2,1).xlabel('Hz')

ax2 = panl(2,2).select();
plot(fZsampled,Z_angle/(2*pi)*360,'m'), hold on
xlim(ax2,[17,300]);
panl(2,2).ylabel('phase')
panl(2,2).xlabel('Hz')
...
xlim(ax1,[17,300]);
panl(4,1).ylabel('Norm.')
panl(4,1).xlabel('Hz')

ax2 = panl(4,2).select();
plot(fsampled,Teff_over_T,'k'), hold on
xlim(ax2,[17,300]);
panl(4,2).ylabel('T_{eff}/T')
panl(4,2).xlabel('Hz')
