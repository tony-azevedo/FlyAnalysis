...
fig = figure;
set(fig,'color',[1 1 1],'position',[42 558 1560 420])
pnl = panel(fig);
...
for cnt = 1:length(analysis_cell)
    ...
    line([0],[peak_ctrl-bsline_ctrl],...
        'parent',ax,...
        'color',[0 0 0],'linestyle','none',...
        'marker','o','markeredgecolor',[0 0 0],'markerfacecolor','none');
...
ylim(pnl(length(analysis_cell)+1,1).select(),[0 6.3]);
...
%ylim(pnl(length(analysis_cell)+1,2).select(),[0 6.3]);
