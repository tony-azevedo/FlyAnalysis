trial = load('E:\Data\181210\181210_F1_C1\EpiFlash2CB2T_Raw_181210_F1_C1_5.mat');
...
switch trial.params.mode_2
    case 'VClamp'
        line(x,trial.current_2,'parent',ax1,'color',[1 .2 .2],'tag','');
        ylabel(ax1,'I (pA)'); %xlim([0 max(t)]);
    case 'IClamp'
        line(x,trial.voltage_2,'parent',ax1,'color',[1 .2 .2],'tag','');
...
for cl = 1:size(trial.clustertraces,2)
    ls = line(x(t2.exposure2),(trial.clustertraces(:,cl)-trial.F0_clustertraces(cl))/trial.F0_clustertraces(cl),'parent',ax2,'tag','');
    ls.Color = clrs(cl,:);
end
