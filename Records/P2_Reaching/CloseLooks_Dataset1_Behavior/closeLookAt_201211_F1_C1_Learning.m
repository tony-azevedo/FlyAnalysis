...
T{1,:} = {'201211_F2_C1', 'WT_berlin',            'pilot', 'LEDArduinoFlashControl','empty'};
...
TP.Target_min = target(1,:)';
TP.Target_max = target(2,:)';
...
%% Finds blocks when the target changes.
% assume some rest trials

targetchanges = find(diff(target(1,:)));
blocks = [[1; targetchanges(:)+1],[targetchanges(:);size(target,2)]];
blcktable = array2table(blocks,'VariableNames',{'b_start','b_end'});
