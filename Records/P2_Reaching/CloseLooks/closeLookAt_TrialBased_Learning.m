...
% T{1,:} = {'210108_F3_C1', 'Hot-Cell-Gal4',            'pilot', 'LEDArduinoFlashControl','empty'};
...
%T{1,:} = {'210111_F2_C1', 'Hot-Cell-Gal4',            'pilot', 'LEDArduinoFlashControl','empty'};
%T{1,:} = {'210111_F3_C1', 'Hot-Cell-Gal4',            'pilot', 'LEDArduinoFlashControl','empty'};
%T{1,:} = {'210111_F4_C1', 'Hot-Cell-Gal4',            'pilot', 'LEDArduinoFlashControl','empty'};
...
dsmp = quantile(diff(find(diff(trial.probe_position)>0)),.1); 

% does the stim dur change?
[bigblcks,changepnts,indices] = unique(TP.durSweep);
indx = order(changepnts);
...
if length(resttrialend) == length(resttrialstart)
    ...
    blocks = [[1; resttrialend + 1],resttrialstart-1];
end
