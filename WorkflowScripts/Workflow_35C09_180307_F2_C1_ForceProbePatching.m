...
clear trials
trials{1} = 1:30; % beautiful vidoes of the leg with no bar, but not analyable yet
trials{2} = 31:60; 
Nsets = length(trials);
...
% %%
% brightSpots2Smooth = trial.brightSpots2Smooth;
% 
% for tr_idx = trialnumlist(1:end)
%     trial = load(sprintf(trialStem,tr_idx));
%     trial.brightSpots2Smooth = brightSpots2Smooth;
%     fprintf('Saving bright spots to smooth in trial %s\n',num2str(tr_idx))
%     save(trial.name,'-struct','trial')
% end
