function [out,vars_skeleton] = spikeDetection_forSweta_temp(data,vars_initial)
%%
%                     fs: 10000
...
if strcmp(newbutton,'Yes')
    ...
    if isempty(spikes_detected)
        out.spikes = spikes_detected;
        out.spikes_uncorrected = uncorrectedSpikes;
        out.spikeDetectionParams = vars;
        out.spikeSpotChecked = 0;

        % save(trial.name, '-struct', 'trial');
        fprintf('Saved Spikes (%d) and filter parameters saved\n',numel(out.spikes));
        return
    end
    out.spikes = spikes_detected + start_point;
    out.spikes_uncorrected = uncorrectedSpikes + start_point;
    out.spikeDetectionParams = vars;
    out.spikeSpotChecked = 0;
...
if strcmp(newbutton,'Cancel')
    ...
    function varargout = detectSpikes
        ...
        axis(ax_main,'off');
        title(ax_main,'\tWhen done moving threshold, hit ''space'' or any key');
