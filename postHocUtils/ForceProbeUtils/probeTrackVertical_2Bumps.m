...
plot(ax1,locs(1),pks(1),'b+'); 
plot(ax1,locs(2),pks(2),'k+'); 
plot(ax2,evalpnts_x(locs),pks,'ro'); 
plot(ax2,evalpnts_x(locs(1)),pks(1),'b+'); 
plot(ax2,evalpnts_x(locs(2)),pks(2),'k+'); 
...
ylim(ax2,[-filtered_mu(righthash)/8 filtered_mu(righthash)*4])
...
plot(ax1,filtered_frame_mu,'k'); 
plot(ax2,evalpnts_x,filtered_frame_mu,'k'); 
...
%% Now find the peaks of the bar(s)
x2fit = 1:length(filtered_frame_mu);

y2fit = filtered_frame_mu-filtered_frame_mu(righthash);
y2fit(y2fit<0)=0;
y2fit(x2fit<trough)=0;
...
coef = lsqcurvefit(@doubleGaussian_bar_1at0,coef0,x2fit,y2fit,lb,ub);
coef = lsqcurvefit(@doubleGaussian_bar_1at0,coef,x2fit,y2fit,lb,ub);

plot(ax1,x2fit,y2fit)
plot(ax1,x2fit,doubleGaussian_bar_1at0(coef,x2fit))

%%

[pks,locs] = findpeaksWithCurrentModel(filtered_frame_mu(trough:righthash));
...
% bar_threshold_right =27/32;% 23/32
% bar_threshold_left = 27/32; %19/32
...
%     end
    ...
    [pks,locs] = findpeaksWithCurrentModel(filtered_frame_mu-dark);
    ...
    forceProbePosition(:,kk,1) = x_hat.*(com(1));
    forceProbePosition(:,kk,2) = x_hat.*(com(2)-1);
