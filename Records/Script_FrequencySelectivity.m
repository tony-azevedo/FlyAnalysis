...
if save_log
    ...
    for c_ind = 1:length(analysis_cell)
        if ~isempty(analysis_cell(c_ind).PiezoSineTrial)
            hasPiezoSineName{end+1} = analysis_cell(c_ind).name;
            genotype{end+1} = analysis_cell(c_ind).genotype;
        end
    end
...
%% Example High Pass Neuron

%% Plot only relevant displacements (0.1 -> 0.05, .2->.15 .4->.5)
