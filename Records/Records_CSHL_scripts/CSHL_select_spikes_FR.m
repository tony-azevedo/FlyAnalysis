...
panl(2,1).pack('v',{1/3 1/3 1/3})  % spikes, emg, bar 1
panl(2,2).pack('v',{1/3 1/3 1/3})  % spikes, emg, bar 2

panl.margin = [18 16 10 10];
panl.descendants.margin = [10 4 10 4];
...
for n = 1:2
    n_sp_ax = panl(2,n,1).select();
    n_emg_ax = panl(2,n,2).select();
    n_bar_ax = panl(2,n,3).select();
    ...
    for tr = trialnumlist
        ...
        emgtrjct = plot(n_emg_ax, t(trjct_twin)-trial.spikes(1),trial.current_2(trjct_twin),'color',clr_sp);
        
        brtrjct = plot(n_bar_ax, ft(trjct_win)-trial.spikes(1),CoM(trjct_win),'color',[1 1 1]);
...
savePDF(N_spikes_fi,savedir,[],sprintf('select_fr_trjct_%s_%d-%d',ID,trialnumlist(1),trialnumlist(2)))
