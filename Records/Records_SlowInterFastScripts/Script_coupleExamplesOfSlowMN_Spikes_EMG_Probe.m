...
raster_ax.TickDir = 'out';
raster_ax.XLim = t_i_f;
raster_ax.YLim = [min(trials(tr).current_2(:)) max(trials(tr).current_2(:))];
raster_ax.XTick = []; 
hold(raster_ax,'on')
...
plot(raster_ax, t(t_win),trials(tr).current_2(t_win),'color',[.7 0 0])
